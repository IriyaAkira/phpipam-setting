...
if "${DOCKER_BIN}" compose exec -T ${IPAM_DATABASE_HOST} \
  ...
  > "${BACKUP_FILE}" 2>&1; then
    echo "$(date '+%Y-%m-%d %H:%M:%S'): DB dump completed successfully." | tee -a "${LOG_FILE}"
else
    echo "$(date '+%Y-%m-%d %H:%M:%S'): WARNING: Could not create DB dump. The container may not be found or running. Continuing..." | tee -a "${LOG_FILE}"
fi
